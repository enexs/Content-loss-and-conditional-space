...
#
#
# Different than the original implementation here we used one big generator
# The size of the generator is approximately doubled the one used in Pix2Pix
...
def decoder_block(layer_in, skip_in, n_filters, dropout=True):
	...
	# 1BIG_gen.py den tek farki bu satir
	g = Activation('relu')(g)
...
def train(d_model, g_model, gan_model, dataset, n_epochs=100, n_batch=1):
	...
	for i in range(n_steps):
		...
		g_loss, discrLoss, L1loss = gan_model.train_on_batch(X_realA, [y_real, X_realB])
		# summarize performance
		L1lossList.append(L1loss)
		print('>%d, d1[%.3f] d2[%.3f] g[%.3f] s1[%.3f] s2[%.3f]' % (i+1, d_loss1, d_loss2, g_loss, discrLoss, L1loss))
		# summarize model performance
		if (i+1) % (bat_per_epo * 10) == 0:
			summarize_performance(i, g_model, dataset)
	# print(L1lossList)
	file_name_for_D = "L1_loss_file_1BIG_gen_activation/L1_loss_1BIG_gen_activation.mat"
	sio.savemat(file_name_for_D, {'vect': L1lossList})
# load image data
...
g_model.summary()
plot_model(g_model, to_file='g_model.png', show_shapes=True)
