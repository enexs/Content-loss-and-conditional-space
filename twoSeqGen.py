...
def define_discriminator(image_shape, name='name'):
...
def define_gan(g_model_1, g_model_2, d_model, image_shape):
	...
	dis_out = d_model([in_src, gen_2_out])
	# src image as input, generated image and classification output
	model = Model(in_src, [dis_out, gen_1_out, gen_2_out])
	# compile model
	opt = Adam(lr=0.0002, beta_1=0.5)
...
def generate_fake_samples(g_model_1, g_model_2, samples, patch_shape):
	...
	X1 = g_model_1.predict(samples)
	X2 = g_model_2.predict(X1)
...
def train(d_model, g_model_1, g_model_2, gan_model, dataset, n_epochs=100, n_batch=1):
	...
	for i in range(n_steps):
		...
		# generate a batch of fake samples
		_, X_fakeB_2, y_fake = generate_fake_samples(g_model_1, g_model_2, X_realA, n_patch)
		# update discriminator for real samples
		d_loss1 = d_model.train_on_batch([X_realA, X_realB], y_real)
		# update discriminator for generated samples
		d_loss2 = d_model.train_on_batch([X_realA, X_fakeB_2], y_fake)
		# update the generator
		g_loss, discrLoss, L1loss_1, L1loss_2 = gan_model.train_on_batch(X_realA, [y_real, X_realB, X_realB])
		# summarize performance
		L1lossList_1.append(L1loss_1)
		L1lossList_2.append(L1loss_2)
		print('>%d, d1[%.3f] d2[%.3f] g[%.3f] s1[%.3f] s2[%.3f] s3[%.3f]' % (i+1, d_loss1, d_loss2, g_loss, discrLoss, L1loss_1, L1loss_2))
		# summarize model performance
		if (i+1) % (bat_per_epo * 10) == 0:
			summarize_performance(i, g_model_1, g_model_2, dataset)
	# print(L1lossList)
	file_name_for_1 = "L1_loss_file_2generator/L1_loss_2generator1.mat"
	file_name_for_2 = "L1_loss_file_2generator/L1_loss_2generator2.mat"
...
# define the models
d_model = define_discriminator(image_shape, name='disc')
d_model.summary()
...
gan_model = define_gan(g_model_1, g_model_2, d_model, image_shape)
gan_model.summary()
# plot_model(gan_model, to_file='gan_model.png', show_shapes=True)

# train model
train(d_model, g_model_1, g_model_2, gan_model, dataset)